...
async def get_people(people_id):
    ...
    resource = await session.get(f'http://swapi.dev/api/people/{people_id}')
...
async def check(check_list, key=None):
    ...
    if len_film > 0:
        for i in range(len_film):
            if key is not None:
                film_n = check_list[i][key]
            else:
                return [{"None": "None"}]
...
async def go():
    async with engine.begin() as con:
        ...
        for ids_chunk in chunked(range(1, 91), MAX_CHUNK_SIZE):
            ...
            for i in range(people_json_list):
                ...
                if "detail" not in people_json_list[i][id_p]:
                    ...
                    species_final = ''.join(
                        '{}: {}, '.format(key, value)
                        for key, value in species_people[0].items()
                    )
                    ...
                    starships_people = await check(starships, "name")
                    ...
                    p_d = {
                            ...
                            "heiht": people_json_list[i][id_p]["height"],
                            ...
                            "vehicles": str(vehicles_peoplr[0]),
                    }
